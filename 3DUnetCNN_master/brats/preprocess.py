...
def convert_brats_data(brats_folder, out_folder, overwrite=False, no_bias_correction_modalities=("flair",)):
    ...
    for subject_folder in glob.glob(os.path.join(brats_folder, "*", "*")):
        if os.path.isdir(subject_folder):
            ...
            movingfile = os.path.join(subject_folder, 't1ce.nii.gz')
            referfile = '/Users/xiaoxiaozhou/Desktop/Research/REU UNet 2D/preprocessed/HGG/Brats18_CBICA_AOZ_1/t1ce.nii.gz'
            outputFileName = os.path.join(new_subject_folder, 't1ce.nii.gz')

            fixed = sitk.ReadImage(referfile, sitk.sitkFloat32)
            ...
            writer.Execute(moving)
            # print(subject_folder)
            # sourcedir = os.path.join(subject_folder, 'truth.nii.gz')
            # targetdir = os.path.join(new_subject_folder, 'truth.nii.gz')
            # shutil.copy(sourcedir, targetdir)


if __name__ == "__main__":
    convert_brats_data("/Users/xiaoxiaozhou/Desktop/Research/3DUnetCNN-master/preprocessed", "/Users/xiaoxiaozhou/Desktop/Research/3DUnetCNN-master/preprocessed_histmatch")