...
from model2d import custom_unet
...
config["all_modalities"] = ["tice_hist", "truth"]
...
config["data_file"] = os.path.abspath("bratsnew_data.h5")
...
def fetch_training_data_files():
    ...
    patientpath = Path(r'/Users/xiaoxiaozhou/Desktop/Research/3DUnetCNN_master/datafor3dtraining/training')
    for subject_folder in glob.glob(os.path.join(patientpath, "*")):
        subject_files = list()
        # for modality in config["training_modalities"] + ["truth"]:
        for image in glob.glob(os.path.join(subject_folder, "*")):
            base = os.path.basename(image)
            if base == 'tice_hist.nii.gz':
                subject_files.append(os.path.join(subject_folder, base))
            elif base == 'truth.nii.gz':
                subject_files.append(os.path.join(subject_folder, base))
...
def main(overwrite=False):
    ...
    if overwrite or not os.path.exists(config["data_file"]):
        ...
        write_data_to_file(training_files, config["data_file"], image_shape=config["image_shape"])          #config["image_shape"] = (144, 144, 144)
    ...
    train_model(model=model,
                model_file=config["model_file"],                        #config["model_file"] = os.path.abspath("tumor_segmentation_model.h5")
