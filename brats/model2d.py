...
def custom_unet(config) -> Model:
    ...
    def create_decoding_block(curr_depth: int) -> None:
        ...
        merge = concatenate([Conv3DTranspose(filters=n_filters, kernel_size=config['decoder_kernel_size'],
                             ...
                             layers[connected_layer.format(curr_depth, config['segmentation_levels'])]], axis=1,
                            name=l_name)
    ...
    # Create input layer
    name = "INPUT"
    inputs = Input(shape=(config['input_channels'], config['input_size'][0], config['input_size'][1], config['input_size'][2]), name=name)
